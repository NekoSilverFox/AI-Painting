...
![image-20230816220804943](doc/pic/image-20230816220804943.png)

## 文生图-高清修复（Hi-Res Fix）

在 “文生图” 中有一个高清修复的按钮。**高清修复也叫高分辨率修复、超分辨率**。 他的本质就是把这张低分辨率的图片img2img，打回浅空间，重新去噪。

**如果在文生图时，出现的多人多头等现象就可以考虑在生存图片的时候开启高分辨率修复，但是高清修复很依赖算力，因为他画一次，就相当于在常规情况下画二到三次，耗费时间过长。**所以一个==技巧==是，**在低分辨率下出图，然后找一张好的图片借助固定随机种子的方式进行高清修复**



![image-20230816221029658](doc/pic/image-20230816221029658.png)

- `高清修复采样次数`：高清修复需要经过一次重绘，因此需要设置采样步数。保持默认的0数值，它会和我们设置的采样迭代次数(20)保持一致
- `重回幅度`：相当于图生图里面的重绘幅度， 如果想保证图片不大幅度发生变化，不要超过**0.5**。如果AI 重绘添油加醋过严重那就需要降低重绘幅度
- `放大算法`：各种算法其实差不多
    - 带有 `GAN（生成式对抗网络）` 系列的算法致力于更好地还原图像特征，保留细节会多一些，**使用GAN系列时重绘幅度不要太高，一般0.2-0.5**。如果是二次元，最好使用 `R-ESRGAN 4x+ Anime6B`
    - `Latent` 系列会进一步把图像打回潜空间内重画，不容易变形。但**Latent系列重绘幅度低了会模糊，一般0.5-0.7**



## 图生图-SD 放大（SD Upscale）

在 *设置->放大->图生图放大算法*里面可以设置图生图的放大算法。

**在 `脚本` 里加入 SD 放大：**

![image-20230816230125601](doc/pic/image-20230816230125601.png)

**原理：**

**图生图中 SD 放大的原理是将原图切割成 `缩放系数 x 缩放系数` 倍，然后针对切割后的每个图像块进行重新绘制。** 而 `图块重叠的像素(Tile overlap)` 就是块与块之间增加的缓冲像素带。



**使用：**

1. 将**宽度和高度**都先**加**上 `图块重叠的像素(Tile overlap)`。比如原来高度和宽度是 `500x500`，**图块重叠的像素**是 64，那么我们需要将高度和宽度设置成 `(500+64)x(500+64)` 也就是 `564x564`
2. 根据需求选择缩放系数
3. 选择放大算法，真人可以选择 `R-ESRGAN 4x+`，二次元选择 `R-ESRGAN 4x+ Anime6B`
4. 生成

如果画面出现添油加醋或者奇怪的东西，需要：**减小重绘幅度、增大`图块重叠的像素(Tile overlap)`**



**优势：**

- 可以突破内存限制获得更大的分辨率（不受缓存限制，最高可达4倍宽高）
- 画面精细度高，对细节的丰富效果出色
